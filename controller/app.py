...
ERROR_SERVER = 500
ERROR_ARGUMENTS = 400
# ID not found in database, must request new ID with /getLightID
ERROR_UNKNOWN_ID = 401
ERROR_NOT_FOUND = 404
STATUS_OK = 200
...
def removeLightID():
  ...
  with lock:
    with open(JSON_FILEPATH, "r") as json_file:
      ...
      intersection_id = traffic["light_map"][request.json["uuid"]
                                             ]["intersection_id"]
...
def getLightState(uuid):
  ...
  time = (monotonic() * 1000 -
          intersection["timing_offset_ms"]) % state_durations[-1]
  ...
  result = {"id": uuid, "intersection_id": intersection_id, "direction": position,
            "state": reported_state, "remaining_ms": state_durations[state] - time}
...
def register(uuid):
  ...
  if uuid not in traffic["light_map"]:
    traffic["light_map"][uuid] = {"id": uuid, "intersection_id": -1,
                                  "direction": "none", "state": STATE_UNASSIGNED, "remaining_ms": 5000}
